...
def test_agent_policy():
    """Test agent policy coordinator."""
    print("\n" + "="*80)
    print("TEST 4: Agent Policy Coordinator")
    print("="*80)

    # Test 1: Domain detection
    test_cases = [
        ("Create a new PySpark notebook", "notebook"),
        ("Explore lakehouse tables", "lakehouse"),
        ("Validate my notebook code", "notebook"),
        ("List all tables in lakehouse", "lakehouse"),
    ]

    print("\n✓ Testing domain detection:")
    for intent, expected_domain in test_cases:
        detected = agent_policy._detect_domain(intent, "auto")
        status = "✓" if detected == expected_domain else "✗"
        print(f"  {status} '{intent[:40]}...' → {detected}")

    # Test 2: Template inference
    print("\n✓ Testing template inference:")
    test_intents = [
        ("Create ETL pipeline notebook", "etl"),
        ("Create machine learning notebook", "ml"),
        ("Create analytics notebook", "analytics"),
        ("Create streaming data notebook", "streaming"),
    ]

    for intent, expected_template in test_intents:
        template = agent_policy._infer_template_from_intent(intent)
        status = "✓" if template == expected_template else "✗"
        print(f"  {status} '{intent}' → {template}")

    # Test 3: Name extraction
    print("\n✓ Testing name extraction:")
    test_intents = [
        ("Create notebook called 'Sales Analysis'", "Sales Analysis"),
        ("Create notebook named \"ETL Pipeline\"", "ETL Pipeline"),
        ("Create 'Test Notebook' notebook", "Test Notebook"),
    ]

    for intent, expected_name in test_intents:
        name = agent_policy._extract_name_from_intent(intent)
        status = "✓" if name == expected_name else "✗"
        print(f"  {status} Extracted: '{name}' from '{intent[:40]}...'")

    # Test 4: Get available workflows
    workflows_info = agent_policy.get_available_workflows()
    print(f"\n✓ Available workflows: {workflows_info['total_count']}")
    print(f"✓ Categories: {', '.join(workflows_info['categories'])}")

    print("\n✅ Agent Policy Coordinator tests passed!")


def test_orchestrator_logic():
    """Test orchestrator suggestion logic."""
    print("\n" + "="*80)
    print("TEST 5: Orchestrator Logic")
    print("="*80)

    # Test notebook orchestrator suggestions
    print("\n✓ Testing notebook orchestrator suggestions:")

    test_cases = [
        ("etl", None, None, "ETL template"),
        ("ml", None, None, "ML template"),
        ("etl", {'has_errors': True}, None, "ETL with errors"),
        ("basic", None, {'performance_score': 65}, "Basic with low performance"),
    ]

    for template, validation, performance, description in test_cases:
        suggestions = notebook_orchestrator._suggest_next_steps(
            template_type=template,
            validation=validation,
            performance=performance
        )
        print(f"  {description:30s} → {len(suggestions)} suggestions")
        if suggestions:
            print(f"     Top: [{suggestions[0]['priority']}] {suggestions[0]['tool']}")

    # Test lakehouse orchestrator suggestions
    print("\n✓ Testing lakehouse orchestrator suggestions:")
    ...
    for table_count, description in test_cases:
        suggestions = lakehouse_orchestrator._suggest_next_steps(
            workspace='Test',
            lakehouse='TestLH',
    ...
    print("\n✅ Orchestrator Logic tests passed!")
...
def run_all_tests():
    """Run all tests."""
    try:
        print("\n🚀 Starting Agentic Logic Test Suite...")

        test_tool_relationships()
